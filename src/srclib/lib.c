...
static char *lib_sizes[] = {"bytes", "Kb", "Mb", "Gb", "Tb", "Pb", "Eb", "Zb", "Yb", "Bb", "GPb"};
...
lib_time_t lib_normalize_time(double seconds) {
    ...
    result.seconds += seconds - ((double)(result.years * SECONDS_PER_YEAR) + (double)(result.days * SECONDS_PER_DAY) +
                                 (double)(result.hours * SECONDS_PER_HOUR) +
                                 (double)(result.minutes * SECONDS_PER_MINUTE) + result.seconds);
...
void lib_new_line(void) { lib_printf(NEW_LINE); }
...
lib_time_t lib_read_elapsed_time(void) { return lib_normalize_time(lib_span); }
...
char *lib_trim(char *str, const char *seps) { return lib_ltrim(lib_rtrim(str, seps), seps); }
