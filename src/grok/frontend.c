...
void fend_on_definition(void) { fend_composition = apr_array_make(fend_pool, ARRAY_INIT_SZ, sizeof(info_t *)); }
...
void fend_on_literal(char *str) { prfend_add_part(str, NULL, part_literal); }

void fend_on_grok(macro_t *macro) { prfend_add_part(macro->name, macro->property, part_reference); }
...
char *fend_strdup(char *str) { return apr_pstrdup(fend_pool, str); }

apr_hash_t *fend_get_patterns(void) { return fend_definition; }
